...
const useStyles = makeStyles((theme) => ({
  root: {
    ...
    backgroundImage:
      "url('https://images.unsplash.com/photo-1641343293300-0fbdd30916ce?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTIzN3x8d2hpdGV8ZW58MHx8MHx8fDA%3D&auto=format&fit=crop&w=600&q=60')", // Add your image path here
...
function ReturnPart(props) {
  ...
  const searchPart = () => {
    WSParts.getPart(partInput).then((res) => {
      setPart(res.body.data);
    });
  };
  useEffect(() => {
    async function fetchData() {
      const res = await WSPicktickets.getBins(part.code, props.store);
