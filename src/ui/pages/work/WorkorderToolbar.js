...
class WorkorderToolbar extends Component {
    ...
    emailWorkorderHandler() {
        window.open("mailto:?Subject=Work Order Number " + this.props.workorder.number + "&body="+process.env.REACT_APP_EAM_URL+"/web/base/logindisp?tenant=DS_MP_1&FROMEMAIL=YES&SYSTEM_FUNCTION_NAME=WSJOBS&workordernum="+this.props.workorder.number+"&organization=ESS", '_self');
    }
    ...
    showInExtendedHandler() {
        //CERN- they read from EAM database, we configure as part of build
        ...
        const extendedLink = process.env.REACT_APP_EAM_URL+"/web/base/logindisp?tenant=DS_MP_1&FROMEMAIL=YES&SYSTEM_FUNCTION_NAME=WSJOBS&workordernum="+this.props.workorder.number+"&organization=ESS";
    ...
    isDismacVisible() {
        return this.props.applicationData.EL_DMUSG ? (this.props.applicationData.EL_DMUSG.includes(this.props.userGroup)) : false;
    }
