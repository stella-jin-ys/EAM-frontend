...
function AddActivityDialog(props) {
  ...
  let init = () => {
    setLoading(true);
    WSWorkorders.initWorkOrderActivity(props.workorderNumber).then(
      (response) => {
  ...
  let onTaskCodeChanged = (taskcode) => {
    ...
    WSWorkorders.getTaskPlan(taskcode)
      .then((response) => {
        ...
        updateFormValues(
          "peopleRequired",
          taskPlan.peopleRequired === null ? "" : taskPlan.peopleRequired
        );
        updateFormValues(
          "estimatedHours",
          taskPlan.estimatedHours === null ? "" : taskPlan.estimatedHours
        );
        updateFormValues(
          "tradeCode",
          taskPlan.tradeCode === null ? "" : taskPlan.tradeCode
        );
      })
