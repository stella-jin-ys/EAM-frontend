...
class WS {
    ...
    getSearchLogisticsData(keyword, config = {}) {
        keyword = encodeURIComponent(keyword);
        return ajax.get(process.env.REACT_APP_BACKEND + '/index/logistics?s=' + keyword, config);
    }

    getSearchLogisticsSingleResult(keyword, config = {}) {
        return ajax.get(process.env.REACT_APP_BACKEND + '/index/logisticssingleresult?s=' + keyword, config);
    }
